if { ![project_exists {heatingsys}] } {
     project_new {heatingsys}
} else {
     project_open {heatingsys}
}
set_project_settings -cmp {heatingsys}
if { ![project_settings_exist -sim heatingsys] } {
        set_project_settings -sim heatingsys
}
set_global_assignment -section_id {heatingsys} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -section_id {heatingsys} -name {EDA_SIMULATION_TOOL} {MODELSIM (VHDL OUTPUT FROM QUARTUS II)}
set_global_assignment -name {EDIF_FILE} {heatingsys.edf}
...
set_global_assignment -section_id {heatingsys} -name {DEVICE} {EP2C35F672C7}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
if { [file exists {heatingsys_pnr_constraints.tcl}] } {
  source heatingsys_pnr_constraints.tcl
}
